...
exports.handler = vandium.generic()
  .handler( (event, context, callback) => {
    ...
    var sql = "SELECT count(*) as api_count FROM apis a WHERE ID IS NOT NULL";
    
    var sql_search = '';
    if(tag != ''){
       sql_search = " AND a.tags LIKE '%" + tag + "%'";
    }
    ...
    connection.query(sql, function (error, results1, fields) { 
      ...
      connection.query(sql2, function (error, results2, fields) {
        ...
        let links = {};
        links.self = '/tag/apis/?tag=' + tag + '&page=' + page + '&limit=' + limit;
        if(page!=0){
          links.first = '/search/apis/?tag=' + tag + '&page=-0&limit=' + limit;
          links.prev = '/search/apis/?tag=' + tag + '&page=' + page-1 + '&limit=' + limit;
        }
        
        if(total_pages > 1){
          links.next = '/search/apis/?tag=' + tag + '&page=' + page+1 + '&limit=' + limit;
        }
        links.last = '/search/apis/?tag=' + tag + '&page=' + total_pages + '&limit=' + limit;
