...
exports.handler = vandium.generic()
  .handler( (event, context, callback) => {
    ...
    var sql = "SELECT count(*) as api_count FROM apis a INNER JOIN maintainers m ON a.openapi_url = m.openapi_url WHERE m.FN LIKE '%" + maintainer + "%' OR m.name LIKE '%" + maintainer + "%'";
    connection.query(sql, function (error, results1, fields) { 
      
      const api_count = results1[0].api_count;
    
      let sql2 = "SELECT a.name,a.description,a.image,a.baseURL,a.humanURL,a.openapi_url,a.tags,GROUP_CONCAT(CONCAT(p.type,'~', p.url)) as properties FROM apis a INNER JOIN maintainers m ON a.openapi_url = m.openapi_url INNER JOIN properties p ON a.baseURL = p.api_base_url WHERE m.FN LIKE '%" + maintainer + "%' OR m.name LIKE '%" + maintainer + "%' GROUP BY a.name LIMIT " + page + "," + limit;
      ...
      connection.query(sql2, function (error, results2, fields) {
        ...
        let links = {};
        links.self = '/search/apis/?maintainer=' + maintainer + '&page=' + page + '&limit=' + limit;
        if(page!=0){
          links.first = '/search/apis/?maintainer=' + maintainer + '&page=-0&limit=' + limit;
          links.prev = '/search/apis/?maintainer=' + maintainer + '&page=' + page-1 + '&limit=' + limit;
        }
        
        if(total_pages > 1){
          links.next = '/search/apis/?maintainer=' + maintainer + '&page=' + page+1 + '&limit=' + limit;
        }
        links.last = '/search/apis/?maintainer=' + maintainer + '&page=' + total_pages + '&limit=' + limit;
